...
struct s_idt_entry idt_entries[256];
struct s_idt_ptr  idt_ptr;
...
void initIDT(){
	...
	idt_ptr.limit = (sizeof(struct s_idt_entry) * 256) - 1;
	idt_ptr.base = (uint32_t)&idt_entries;
	ft_memset(&idt_entries, 0, (sizeof(struct s_idt_entry) * 256));
...
void setIdtGate(uint32_t num, uint32_t base,uint32_t sel,uint8_t flags){
	idt_entries[num].first_offset =  base & 0xFFFF;
	idt_entries[num].second_offset = (base >> 16) & 0xFFFF;
...
void isr_handler(struct InterruptRegister *regs){
	...
	printf("here\n");
	if (regs->int_nbr < 32){
		printf("%s\n", exception_messages[regs->int_nbr]);
		printf("Exceptions system halted");
		for(;;);
	}
...
void irq_handler(struct InterruptRegister *regs){
	void (*handler)(struct InterruptRegister *r);

	handler =  irq_routines[regs->int_nbr - 32];
